...
use crate::{AppState, Buyer, Round, THIRD_ROUND_RATE};

#[derive(Accounts)]
pub struct BuyInThirdRound<'info> {
    #[account(mut)]
...
impl<'info> BuyInThirdRound<'info> {
...
pub fn buy_in_third_round_handler(ctx: Context<BuyInThirdRound>, amount: u64) -> Result<()> {
    token::transfer(ctx.accounts.transfer_ctx(), amount)?;
    let buyer: &mut Box<Account<Buyer>> = &mut ctx.accounts.buyer;
    let round: &mut Box<Account<Round>> = &mut ctx.accounts.third_round;
    if buyer.total_bought3 == 0 {
        round.buyer_count += 1;
    }
    buyer.total_paid += amount;
    let offer_amount: u64 = THIRD_ROUND_RATE * amount / 10_u64.pow(ctx.accounts.currency_mint.decimals as u32) * 10_u64.pow(ctx.accounts.mint.decimals as u32);
    buyer.total_bought3 += offer_amount;
