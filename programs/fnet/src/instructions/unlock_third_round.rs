...
#[derive(Accounts)]
pub struct UnlockThirdRound<'info> {
    #[account(mut)]
    ...
    #[account(mut, constraint = third_round.round_index == 3)]
...
impl<'info> UnlockThirdRound<'info> {
    pub fn transfer_checked_ctx(&self) -> CpiContext<'_, '_, '_, 'info, TransferChecked<'info>> {
        ...
        let cpi_accounts: TransferChecked = TransferChecked {
            from: self.third_round_token.to_account_info(),
...
pub fn unlock_third_round_handler(ctx: Context<UnlockThirdRound>) -> Result<()> {
    let now: i64 = Clock::get().unwrap().unix_timestamp;
    if ctx.accounts.buyer.claimed_amount3 == 0
        && now < ctx.accounts.third_round.end_time + PRESALE_VEST_PERIOD
    {
        return err!(ErrorCode::InvalidClaimTime);
    }
    if ctx.accounts.buyer.claimed_amount3 > 0
        && ctx.accounts.buyer.last_claim3 + PRESALE_VEST_PERIOD < now
    {
        return err!(ErrorCode::InvalidClaimTime);
    }
    let mut amount: u64 = ctx.accounts.buyer.total_bought3
        / DISTRIBUTION_DENOMINATOR as u64
        / PRESALE_UNLOCK_PERCENT as u64;
    let left: u64 = ctx.accounts.buyer.total_bought3 - ctx.accounts.buyer.claimed_amount3;
    if amount > left {
        amount = left;
    }
    let seeds: &[&[u8]; 3] = &[
    ...
    buyer.claimed_amount3 += amount;
    buyer.last_claim3 = now;
    Ok(())
}
