...
#[derive(Accounts)]
pub struct UnlockFirstRound<'info> {
    #[account(mut)]
...
impl<'info> UnlockFirstRound<'info> {
    pub fn transfer_checked_ctx(&self) -> CpiContext<'_, '_, '_, 'info, TransferChecked<'info>> {
        ...
        let cpi_accounts: TransferChecked = TransferChecked {
            from: self.first_round_token.to_account_info(),
...
pub fn unlock_first_round_handler(ctx: Context<UnlockFirstRound>) -> Result<()> {
    let now: i64 = Clock::get().unwrap().unix_timestamp;
    if ctx.accounts.buyer.claimed_amount1 == 0
        && now < ctx.accounts.first_round.end_time + PRESALE_VEST_PERIOD
    {
        return err!(ErrorCode::InvalidClaimTime);
    }
    if ctx.accounts.buyer.claimed_amount1 > 0
        && ctx.accounts.buyer.last_claim1 + PRESALE_VEST_PERIOD < now
    {
        return err!(ErrorCode::InvalidClaimTime);
    }
    let mut amount: u64 = ctx.accounts.buyer.total_bought1
        / DISTRIBUTION_DENOMINATOR as u64
        / PRESALE_UNLOCK_PERCENT as u64;
    let left: u64 = ctx.accounts.buyer.total_bought1 - ctx.accounts.buyer.claimed_amount1;
    if amount > left {
        amount = left;
    }
    let seeds: &[&[u8]; 3] = &[
    ...
    buyer.claimed_amount1 += amount;
    buyer.last_claim1 = now;
    Ok(())
}
